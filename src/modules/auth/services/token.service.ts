...
import { tokenRepository } from "src/modules/auth/repository/token.repository";
...
export const tokenService = {
  ...
  verifyAccessToken(token: string): AccessTokenPayload {
    try {
      const decoded = jwt.verify(token, config.ACCESS_TOKEN_SECRET) as unknown;
  ...
  verifyRefreshTokenSignature(token: string): RefreshTokenPayload {
    try {
      const decoded = jwt.verify(token, config.REFRESH_TOKEN_SECRET) as unknown;
  ...
  async validateRefreshToken(rawToken: string): Promise<RefreshTokenPayload> {
    ...
    const payload = tokenService.verifyRefreshTokenSignature(rawToken);
    const tokenHash = hashToken(rawToken);
